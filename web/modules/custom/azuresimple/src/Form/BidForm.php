...
class BidForm extends FormBase
{
    ...
    public function buildForm(array $form, FormStateInterface $form_state)
    {
        ...
        $nid = $node->nid->value;
        $form = [];
        if ($node->get('field_end_date')->value - microtime(True) < 0) {
        ...
        $messenger = \Drupal::service('messenger');
        $messenger->addMessage($this->t('Remaining Time @currenttime', array('@currenttime' => $this->seconds2human($node->get('field_end_date')->value - microtime(True)))));
    ...
    public function submitForm(array &$form, FormStateInterface $form_state)
    {
        ...
        $messenger = \Drupal::service('messenger');
        if ($node->get('field_end_date')->value - microtime(True) < 0) {
            $message->addMessage($this->t('Sorry the auction is closed.'));
            return;
        }
